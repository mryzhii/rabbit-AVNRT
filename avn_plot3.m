%
%
function avn_plot3(nn,Mode,S,ap,al,bl,t1,y1,y2, ...
...
M4_Small_fig = 0; % Small plot 1/2
M7_first = 0;    % Do not draw 2nd and 3rd panels

plot_height = 350; % 420  % 640; for video 
plot_width =  1500; %870; %450;  % 480; for video

plot_scale = 1.0;    % 1.5; for video
pscale2 = 0.9;
pscale3 = 0.9;

tname0 = {'S1S2, Anterograde conduction','S1S2, Retrograde conduction'};
tname1 = {', FP ablation',', SP ablation'};
...
main_col  = "#333333"; % '#888888'; 
ap_col    = "#333333"; %"#000000";
apS_col   = [0     0.447 0.741]; % "#0072BD"; % [0     0.447 0.741]
apF_col   = "#E50200"; %"#D95319"; % [0.850 0.325 0.098]
pulse_col = "#000000"; %"#900000"; %"#707070"; 
osc_col   = ap_col;
point_col = ap_col; %'#900000'; 
point_col1= ap_col; %'#900000'; 
coltest   = "#009000";
set(groot, 'defaultAxesTickLabelInterpreter','tex'); 

lw1 = 0.5; %0.6; 
ms = 0.1; % 1.5; % normal cell marker
ms2= 5; %4;     Stimulus marker
msp= 3; %2 ;3;  % Pacemaker marker 
mcol = '#555555';
lw2 = lw1; % pacemakers 
lw3 = 0.3;
lw_top = 1;
...
switch Mode
    case {8, 9}
        ...
        xlim2 = plot_end_time;
        xtk = 1.0; %0.1;
        f1.Position =      [30 277 plot_width plot_height];
        nskip = 50; % 25;
    case {81, 82, 83}
        x_pos = -0.065;
        xlim1 = plot_st_time;
    ...
    for j= 1:al     
        for  i= 1:al 
            ...
            if abs(locs_F(n,j)-locs_F(n+1,i)) < min_int
                 ...
                 if (Mode==7 && x(1) < plot_st_time*0.95), break; end
                 if(Mode~=71) % Do NOT Draw  
                  if n < S.iAM3
    ...
    for j=1:al
        for i=1:bl 
            if (abs(locs_S(n,j)-locs_S(n+1,i)) < min_interval(1)) 
                ...
                if (Mode==7 && x(1) < plot_st_time*0.95), break; end
                if (n>=S.iAM3 && n<S.iSlow2+2  && Mode~=71  &&nnn==0)   % DO NOT Draw
...
switch Mode
    ...
    case {83,88, 99}
      plot(tmp_distrib-dt,-S.imp_Retro,'>',...
        'MarkerFaceColor',pulse_col,'MarkerEdgeColor',pulse_col,'MarkerSize',ms2);
    case {89}
      plot(tmp_distrib(1)-dt,-S.imp_Retro,'>',...
        'MarkerFaceColor',pulse_col,'MarkerEdgeColor',pulse_col,'MarkerSize',ms2);
      plot(tmp_distrib(2:end)-dt,-S.imp_Antero,'>',...
        'MarkerFaceColor',pulse_col,'MarkerEdgeColor',pulse_col,'MarkerSize',ms2);
    otherwise
end
...
case {81, 82, 83} %%%%%%%%%%%%%%%%  81, 82, 83 %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    switch Mode
     case 82  %----------------  82 PJC -------------------------------
          text('Units','data','Position',[plot_st_time ypos],'String','PJC', ...
          'Color',[0.9 0 0],'fontsize',fs2,'fontname',fn1);  
     case 81  %----------------  81 PAC -------------------------------
          text('Units','data','Position',[plot_st_time ypos],'String','PAC', ...
          'Color',[0.9 0 0],'fontsize',fs2,'fontname',fn1);  
     case 83  %----------------  83 PVC -------------------------------
          text('Units','data','Position',[plot_st_time ypos],'String','PVC', ...
          'Color',[0.9 0 0],'fontsize',fs2,'fontname',fn1);  
    end
...
case {9,99} %%%%%%%%%%%%%%%%  9 99  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      xpos = 0.5;
      ypos = 2.0;
      txt = ['S1 = ' num2str(tmp_distrib(end-1)-tmp_distrib(end-2),'%.3f') ...
           '    S2 = ' num2str(tmp_distrib(end)-tmp_distrib(end-1),'%.3f') ...
           '   pwidth= ',num2str(ap.pulse_width,'%.4f')  ' s'];
      text('Units','data','Position',[xpos ypos],'String',txt, ...
          'Color',[0 0 0],'fontsize',fs2,'fontname',fn1);

end



if M7_first, return; end
...
switch Mode
case{9} % Anterograde -> Stimuli on the top
    plot(t1(startt:nskip0:endt),y2(S.imp_Antero,startt:nskip0:endt)+del, ...
        'LineWidth',lw,'Color',pulse_col);

case {88, 99} % Retrogarde -> Stimuli on the bottom
    plot(t1(startt:nskip0:endt),y2(S.imp_Retro,startt:nskip0:endt)- ...
        (S.iHB2-4.5)*del , ...
        'LineWidth',lw,'Color',pulse_col);
    plot(xlim1,-S.iHB2*del,'>','MarkerFaceColor',pulse_col, ...
                  'MarkerEdgeColor',pulse_col,'MarkerSize',ms2);
    ylim([-(n_end+1)*del 1]);
